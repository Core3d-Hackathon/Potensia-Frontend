...
import { Mail, MapPin, School, BookOpen, Medal, Star, Zap, ShieldCheck, Award, FileText, Share2, Target, CheckCircle2, Loader } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  // Jika data user tidak ada (belum login / error jaringan)
  if (!user) {
    return (
      <div className="flex justify-center items-center py-40 flex-col gap-4">
        <ShieldCheck className="w-16 h-16 text-zinc-300" />
        <p className="text-zinc-500 font-medium">Gagal memuat profil. Pastikan kamu sudah login.</p>
      </div>
    );
  }

  // Perhitungan Gamifikasi (Dummy Logic untuk tampilan UI berdasarkan Poin)
  const currentPoints = user.points || 0;
  const currentLevel = Math.floor(currentPoints / 100) + 1; // Tiap 100 poin naik 1 level
  const pointsNextLevel = currentLevel * 100;
  const progressPercentage = ((currentPoints % 100) / 100) * 100;
  const pointsNeeded = pointsNextLevel - currentPoints;
        ...
        <div className="px-8 pb-8 relative">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 -mt-16 md:-mt-20 relative z-10">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white shadow-xl overflow-hidden bg-zinc-100 shrink-0 relative flex items-center justify-center font-black text-teal-700 text-5xl">
                {user.imageUrl ? (
                  <img 
                    src={user.imageUrl} 
                    alt={user.name} 
              ...
              <div className="pt-2 md:pt-24 space-y-2">
                ...
                <p className="text-sm font-bold text-teal-700 uppercase tracking-widest flex items-center gap-2">
                  <Star className="w-4 h-4 fill-teal-700" /> Guru Kontributor
                </p>
                <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-500 font-medium mt-3">
                  <span className="flex items-center gap-1.5"><School className="w-4 h-4" /> Instansi Nusantara</span>
                  <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4" /> Indonesia</span>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[2rem] border border-zinc-200/60 shadow-sm p-8">
            ...
            <p className="text-sm text-zinc-600 leading-relaxed">
              Halo! Saya adalah seorang pendidik yang bersemangat mengintegrasikan teknologi ke dalam ruang kelas. Saya percaya bahwa setiap anak memiliki potensi unik yang bisa dimaksimalkan melalui pendekatan pembelajaran yang personal. Mari saling berbagi RPP dan berkolaborasi di Potensia!
            </p>
            <div className="mt-6 flex flex-wrap gap-2">
              {['Pendidik', 'Inovator', 'Teknologi Pendidikan'].map(skill => (
          ...
          {/* Activity Stats (Gamifikasi Tampilan) */}
          <div className="bg-white rounded-[2rem] border border-zinc-200/60 shadow-sm p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-5 rounded-2xl bg-[#eff6ff] border border-blue-100 flex flex-col items-center justify-center text-center gap-2">
                ...
                <span className="text-2xl font-black text-blue-900">{Math.floor(currentPoints / 50) || 0}</span>
                <span className="text-[0.65rem] font-bold text-blue-700 uppercase tracking-widest">RPP Dibagikan</span>
              </div>
              <div className="p-5 rounded-2xl bg-amber-50 border border-amber-100 flex flex-col items-center justify-center text-center gap-2">
                ...
                <span className="text-[0.65rem] font-bold text-amber-700 uppercase tracking-widest">Total XP Terkumpul</span>
              </div>
            </div>
          </div>
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-lg font-bold text-zinc-900 flex items-center gap-2">
                  <Zap className="w-5 h-5 text-amber-500 fill-amber-500" /> Level {currentLevel}
                </h3>
                <span className="bg-[#e8faf4] text-[#00a870] px-3 py-1 rounded-lg text-xs font-black">
                  {currentPoints} XP
                </span>
              </div>
              <p className="text-xs text-zinc-500 font-medium mb-6">Peringkat Global</p>

              {/* Progress Bar */}
              <div className="space-y-2">
                <div className="flex justify-between text-[0.65rem] font-bold text-zinc-400 uppercase tracking-wider">
                  <span>Progress</span>
                  <span>{pointsNeeded} XP Menuju Lv.{currentLevel + 1}</span>
                </div>
                <div className="h-3 w-full bg-zinc-100 rounded-full overflow-hidden border border-zinc-200/50">
                  <div className="h-full bg-gradient-to-r from-[#00a870] to-teal-400 rounded-full relative transition-all duration-1000" style={{ width: `${progressPercentage}%` }}>
          ...
          <div className="bg-white rounded-[2rem] border border-zinc-200/60 shadow-sm p-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4 p-4 rounded-xl border border-zinc-100 bg-zinc-50 hover:bg-white hover:border-[#a7f3d0] hover:shadow-sm transition-all group">
                ...
                <div>
                  <h4 className="font-bold text-sm text-zinc-800 mb-1">Pionir Potensia</h4>
                  <p className="text-[0.65rem] text-zinc-500 leading-relaxed">Bergabung dan menjadi bagian dari komunitas revolusi pendidikan.</p>
                </div>
              </div>
              
              {/* Badge 2 - Unlock if level > 1 */}
              <div className={`flex gap-4 p-4 rounded-xl border transition-all group ${currentLevel > 1 ? 'border-zinc-100 bg-zinc-50 hover:bg-white hover:border-[#a7f3d0] hover:shadow-sm' : 'border-dashed border-zinc-200 bg-zinc-50/50 opacity-60 grayscale'}`}>
                ...
                <div>
                  <h4 className="font-bold text-sm text-zinc-800 mb-1">Guru Berbagi</h4>
                  <p className="text-[0.65rem] text-zinc-500 leading-relaxed">Telah mempublikasikan minimal 1 RPP ke Ruang Komunitas.</p>
                </div>
              </div>
